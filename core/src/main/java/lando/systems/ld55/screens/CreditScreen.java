...
//    private final Animation<TextureRegion> kittenAnimation;
    ...
    public CreditScreen() {
        ...
        titleLabel = new TypingLabel(assets.fontAbandoned, title, 0f, Config.Screen.window_height - 20f);
        titleLabel.setWidth(Config.Screen.window_width);
        titleLabel.setFontScale(1f);

        themeLabel = new TypingLabel(assets.fontZektonSmall, theme, 0f, Config.Screen.window_height - 80f);
...
//        game.audioManager.playMusic(AudioManager.Musics.outro);
    ...
    public void update(float dt) {
        if (Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE) || Gdx.input.isTouched() || !exitingScreen) {
            if (accum > 2f) {
                exitingScreen = true;
