...
import lando.systems.ld55.utils.screenshake.SimplexNoise;
...
public class WindEffect extends Effect {
    ...
    private SimplexNoise noise        = new SimplexNoise(6, 0, 1);
    ...
    protected void onApply(TypingGlyph glyph, int localIndex, float delta) {
        ...
        float noiseX = (float)noise.getNoise(noiseCursorX + indexOffset, 0f);
        float noiseY = (float)noise.getNoise(0f, noiseCursorY + indexOffset);
